...
const Card: React.FC<{ children: React.ReactNode; className?: string }> = ({ children, className }) => (
    <div className={`bg-gray-800 rounded-lg shadow-lg p-6 ${className}`}>{children}</div>
);
...
const Button: React.FC<{ onClick?: () => void; children: React.ReactNode; className?: string; type?: 'button' | 'submit' | 'reset'; variant?: 'primary' | 'secondary' }> = ({ onClick, children, className, type = 'button', variant = 'primary' }) => {
    const baseClasses = "font-bold py-2 px-4 rounded-lg focus:outline-none focus:shadow-outline transition-transform transform hover:scale-105 active:scale-100";
...
const AddStudentModal: React.FC<{ isOpen: boolean; onClose: () => void; }> = ({ isOpen, onClose }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const studentSubjects = getSubjectsForStudent(examType, field);
        ...
        onClose();
        // Reset form
        setName(''); setEmail(''); setPassword(''); setExamType(''); setField('');
    };
...
const StudentDetailPage: React.FC<{student: Student; onBack: () => void}> = ({ student: initialStudent, onBack }) => {
    ...
    const ReportTabButton: React.FC<{name: string, label: string}> = ({ name, label}) => (
        <button onClick={() => setActiveReportTab(name)} className={`px-4 py-2 text-sm font-semibold rounded-md transition-colors ${activeReportTab === name ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>
            {label}
        </button>
    );
    ...
    const ExamTypeFilterButton: React.FC<{ name: 'all' | 'TYT' | 'AYT'; label: string; }> = ({ name, label }) => (
        <button
            ...
            className={`px-4 py-2 text-sm font-semibold rounded-md transition-colors ${examTypeFilter === name ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}
        >
            {label}
        </button>
    );
                ...
                {activeTab === 'reports' && (
                    <div>
                         <div className="p-2 bg-gray-900/50 rounded-lg flex gap-2 mb-6 self-start">
                           ...
                           <ReportTabButton name="konu" label="Konu Raporu" />
                        ...
                        {activeReportTab === 'deneme' && (
                            <div className="grid grid-cols-1 gap-6 animate-fade-in-down">
                                {student.examType === EXAM_TYPES.TYT_AYT && (
                                     <div className="p-2 bg-gray-900/50 rounded-lg flex gap-2 self-start">
